 ...
 *
 * Logica:
 * - update: trova TUTTE le righe con stesso id_prodotto, oppure stesso SKU+descrizione, oppure stesso SKU
 *   con id vuoto / uguale; scrive su tutte (così le copie duplicate non restano sfasate e non servono nuove righe).
 * - insert: se id_prodotto è già sul foglio, aggiorna quella riga (doppio invio); altrimenti append.
 */
function doPost(e) {
  try {
    if (!e || !e.postData) {
    ...
    if (action === 'update') {
      ...
      return jsonOut_({
        ok: false,
        error:
          'Nessuna riga da aggiornare (id/SKU non trovati sul foglio). Controlla colonne id_prodotto e SKU.',
      })
    }
...
function ensureIdColumn_(sh, map) {
  ...
  const c = Math.max(1, sh.getLastColumn()) + 1
  sh.getRange(1, c).setValue('id_prodotto')
  map.id_prodotto = c
}
